...
EmbeddedCliConfig *embeddedCliDefaultConfig(void) {
    ...
    defaultConfig.invitation = "STM32->: ";
...
{
    if(mycli == NULL){
        printf("mycli is null 3\r\n");
    }
    embeddedCliReceiveChar(mycli, c);
}

void uart2Cli_init(void)
{
    config = embeddedCliDefaultConfig();
    config->maxBindingCount = 16;
    mycli = embeddedCliNew(config);
    // mycli = embeddedCliNewDefault();
    if(mycli == NULL){
        printf("mycli is null 1\r\n");
    }
    mycli->writeChar = writeChar;
}

void myCliProcess(void)
{
    if(mycli == NULL){
        printf("mycli is null 2\r\n");
    }
    embeddedCliProcess(mycli);
}
